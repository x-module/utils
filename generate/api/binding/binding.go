...
import (
	...
	"bytes"
	"fmt"
	"github.com/go-xmodule/utils/generate/api/typescriptify"
...
func NewBindings(structPointersToBind []interface{}, exemptions []interface{}, obfuscate bool, routeMap map[string]RouteMap) *Bindings {
	result := &Bindings{
		db:                  newDB(),
	...
	for _, ptr := range structPointersToBind {
		err := result.Add(ptr)
		if err != nil {
			xlog.Logger.Fatal("Error during binding: " + err.Error())
		}
	}
